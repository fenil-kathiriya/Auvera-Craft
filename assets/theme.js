...
theme.HeaderSection = (function() {
  ...
  HeaderSection.prototype = _.assignIn({}, HeaderSection.prototype, {
    ...
    headerHeight : function(){
      ...
      var messageBar = document.querySelector('.store-messager-bar');
      var rootElement = document.querySelector(':root');
      rootElement.style.setProperty("--height-header", header.offsetHeight + "px");
      if(typeof messageBar !== 'undefined' && messageBar !== null){
        rootElement.style.setProperty("--height-message-bar", messageBar.offsetHeight + "px");
      }
        ...
        if (product.variants[0].option1 !== "Default Title"){
          $(options).each(function(i, option) {
            ...
            $(option.values).each((i, value) => {
              $(`.option_qv.${opt}`).append(`<option value="${value}">${value}</option>`);
            });
            //list
                ...
                if (option.name.includes(theme.strings.colorVariant1) || option.name.includes(theme.strings.colorVariant2)) {
                  ...
                  if (variant_img == '') {
                    ...
                    if(qv_color){
                      ...
                      var nvl = value.replace(/&/g, 'amp').replace(/[^a-zA-Z0-9]+/g, '-').toLowerCase();
...
theme.miniCart = (function(){
  ...
  function observeProductRecommendations(customID) {
    ...
    const productRecommendationsSection = document.querySelector('.product-recommendations-in');
    ...
    const handleIntersection = (entries, observer) => {
      ...
      fetch(url)
        ...
        .then(text => {
          ...
          html.innerHTML = text;
          const recommendations = html.querySelector('.product-recommendations-in');
